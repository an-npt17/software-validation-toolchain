...
def main():
    ...
    parser.add_argument(
        "--tool", type=str, required=True, help="Formal verification tool name"
    )
    parser.add_argument(
            ...
            for idx, task in enumerate(tasks):
                ...
                while attempt <= attempt_limit:
                    ...
                    response = form_query_response(
                        query_inst, system_prompt, task_context
                    )

                    code_response, tag = extract_code(response)
                    code_response_time = time.time()
                    print(
                        f"Attemp {attempt} code response: {code_response_time - start}s"
                    )
                    ...
                    code_path = os.path.join(
                        log_dir, f"code_response_{idx + 1}_{attempt}.txt"
                    )
                    ...
                    context_path = os.path.join(
                        log_dir, f"refine_context_{idx + 1}_{attempt}.txt"
                    )
                    ...
                    print(
                        f"Attemp {attempt} coherence response: {coherence_response_time - code_response_time}s"
                    )
                    correctness_path = os.path.join(
                        log_dir, f"coherence_check_{idx + 1}_{str(attempt)}.txt"
                    )
                    save_to_file(
                        correctness_path,
                        coherence_check,
                    ...
                    verification_path = os.path.join(
                        log_dir, f"verification_{idx + 1}_{str(attempt)}.txt"
                    )
                    ...
                    if result["succeed"] and not result["partial"]:
                        ...
                        task_context_path = os.path.join(
                            log_dir, f"task_context_{idx + 1}_{attempt}.txt"
                        )
                        save_to_file(
                            task_context_path, json.dumps(task_context, indent=4)
                        )
                        break
